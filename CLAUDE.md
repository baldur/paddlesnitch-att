...
- [`similar-sections-compare.md`](docs/features/similar-sections-compare.md) — 💭 design 2026-07-20 (spec-driven; not built). Compare similar sections across your **own** paddles as an **ad-hoc race between two derived gate lines**. Click two points on a saved paddle's map → derive a start line + finish line perpendicular to the track (an on-the-fly ATT `point_to_point` course) → race every other own paddle through it, keeping those that cross **both gates in order, same direction** (fastest valid pair; reversed excluded). Match list is reverse-chronological; compare zeroes every racer at the start line and **recalculates pace/500 for that section only** (+ per-500 section splits + map overlay of racers between the gates). **Reuses ATT line-crossing timing** (`processTrace`/`segmentIntersect`/splits from `@paddlesnitch/timing/geo`) — the only new pure code is gate-derivation + a path-similarity **quality score** (heuristic v1, a model-ranking seam for later). No `AnalysisSession` schema change (derives from saved `result.points`). New `apps/analysis/src/lib/similar.ts` + `POST /analyse/api/analyse/similar` (list) & `…/similar/compare` (picked subset). Phased P1 engine+tests → P2 select UI+list → P3 race board. Needs two real same-direction overlapping traces to calibrate thresholds before trusting matches.
