...
**Automated:**
```bash
pnpm test         # 137 tests: parsers + Cognito auth + upload + courses + crew + pace/date + GDPR + password-reset + OTP routes + Lambda triggers
...
pnpm test       # Vitest, 137 tests across 13 files (spawns its own cognito-local on :9230)
pnpm test:watch
```
...
Use **Vitest**. 137 tests across 13 files. Vitest `globalSetup` spawns its own cognito-local on :9230 so auth/upload/courses tests run against the real Cognito SDK surface (no mocks except `next/headers`).
