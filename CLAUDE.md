...
- [`platform-monorepo.md`](docs/features/platform-monorepo.md) — 🚧 planned 2026-07. paddlesnitch → multi-app platform: one Cognito pool + one S3 bucket + one CloudFront, apps under `apps/*` (att, analysis, …) sharing `packages/core` + `packages/timing`. In-place pnpm-workspace migration, path-based routing, per-app `basePath`. Phased (A1–A5).
