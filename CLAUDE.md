...
**Automated:**
```bash
pnpm test         # 112 tests: parsers + Cognito auth + upload + courses + crew + pace/date + GDPR export/delete
...
pnpm test       # Vitest, 112 tests across 10 files (spawns its own cognito-local on :9230)
pnpm test:watch
```
...
Use **Vitest**. 112 tests across 10 files. Vitest `globalSetup` spawns its own cognito-local on :9230 so auth/upload/courses tests run against the real Cognito SDK surface (no mocks except `next/headers`).
