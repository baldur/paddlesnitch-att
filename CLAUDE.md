...
**Automated:**
```bash
pnpm test         # 103 tests: parsers + Cognito auth + upload + courses + crew validation + pace/date helpers
...
pnpm test       # Vitest, 103 tests across 9 files (spawns its own cognito-local on :9230)
pnpm test:watch
```
...
Use **Vitest**. 103 tests across 9 files. Vitest `globalSetup` spawns its own cognito-local on :9230 so auth/upload/courses tests run against the real Cognito SDK surface (no mocks except `next/headers`).
