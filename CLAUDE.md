...
- [`weather-and-river-flow.md`](docs/features/weather-and-river-flow.md) — 🟡 phase 1 (#106). Per-entry weather (Open-Meteo) + river flow (UK EA flood-monitoring) captured at the entry's finish time, against the auto-picked nearest flow station + course start-line midpoint. No API key for either source. Captured best-effort at upload time, backfilled at read time (`enrichTrialConditions`), shown in the leaderboard's expanded row. All in `src/lib/conditions.ts`; display helpers (`weatherCodeLabel`, `compass8`) in `format.ts`.
