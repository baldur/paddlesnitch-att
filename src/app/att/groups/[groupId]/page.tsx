...
import type { GroupMetadata, GroupInvitation, AuthUser } from '@/lib/types'

type Invitation = GroupInvitation

// /att/groups/[groupId] — manage members and invitations.
//
...
export default function GroupDetailPage({
  ...
  const [group, setGroup] = useState<GroupMetadata | null | undefined>(undefined)
  ...
  const loadAll = async () => {
    ...
    const c = await groupRes.json()
    setGroup(c)
    // Only owners + admins can list invitations — try, ignore 403.
    const invRes = await fetch(`/att/api/groups/${groupId}/invitations`)
    if (invRes.ok) {
  ...
  if (!user) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
        ...
        <Link href="/att/auth?next=/att/groups" className="px-6 py-2 bg-[#0369a1] text-white text-xs font-bold tracking-widest hover:bg-[#0284c7] transition-colors">SIGN IN</Link>
      </main>
    )
  }
  if (!group) {
    return (
      <main className="flex-1 flex flex-col items-center justify-center gap-4 px-4">
        <p className="text-sm text-[#64748b]">This group is not visible to you, or doesn&apos;t exist.</p>
  ...
  const role: 'owner' | 'admin' | 'member' = (() => {
    ...
    if (group.adminUserIds.includes(user.id)) return 'admin'
    return 'member'
  })()
      ...
      <div className="flex-1 px-4 py-8 max-w-3xl mx-auto w-full space-y-10">
        ...
        <section>
          <h2 className="text-xs text-[#64748b] tracking-[0.2em] uppercase mb-4">
            Members ({1 + group.adminUserIds.length + group.memberUserIds.length})
          </h2>
          <div className="flex flex-col gap-1.5">
            <MemberRow userId={group.ownerId} role="owner" canKick={false} onKick={() => {}} />
            {group.adminUserIds.map(id => (
              <MemberRow
                key={id}
                userId={id}
                role="admin"
