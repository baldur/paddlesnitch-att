...
export default function LibraryPage() {
  ...
  const load = () => fetch('/att/api/analyse/sessions').then(r => (r.ok ? r.json() : null)).then(d => setSessions(d?.sessions ?? null)).catch(() => setSessions(null))
  useEffect(() => { load() }, [])

  const del = async (id: string) => {
    if (!confirm('Delete this paddle?')) return
    await fetch(`/att/api/analyse/sessions/${id}`, { method: 'DELETE' })
    ...
    <div className="min-h-screen bg-[#0b1220] text-[#e2e8f0] px-4 py-6">
      <div className="max-w-3xl mx-auto">
        <div className="flex items-center justify-between mb-5">
          ...
          <Link href="/att/analyse" className="text-xs tracking-widest text-[#64748b] hover:text-[#e2e8f0]">+ ANALYSE A PADDLE</Link>
        </div>

        {sel.length === 2 && (
          <Link href={`/att/analyse/compare?a=${sel[0]}&b=${sel[1]}`}
        ...
        {sessions === null && <p className="text-sm text-[#64748b]">Sign in to see your saved paddles. <Link href="/att/auth?next=/att/analyse/library" className="text-[#0369a1]">Sign in</Link></p>}
        {sessions && sessions.length === 0 && <p className="text-sm text-[#64748b]">No paddles yet. <Link href="/att/analyse" className="text-[#0369a1]">Analyse your first one →</Link></p>}
        ...
        <div className="flex flex-col gap-2">
          {sessions?.map(s => (
            <div key={s.id} className="border border-[#1e293b] rounded p-3 flex gap-3 items-start">
              ...
              <Link href={`/att/analyse/${s.id}`} className="flex-1 min-w-0">
